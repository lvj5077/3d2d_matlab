...
stdmm = zeros(40,8);
memm = zeros(40,8);
imageNoise = zeros(40,1);
% A_depth
for j = 1:40
    j
    A_depth = 0.01*j;
    imageNoise(j) = A_depth;
    ...
    for testId = 1:testNum
        ...
        NsImg = fixImg;
        NsImg = NsImg + A_depth*wgn(samples,2,0) ;
        ...
        d_base = 3*rand(samples,1)+2;
        d  = d_base;
%         for i = 1:samples
%             d(i) = d(i) + A_depth*wgn(1,1,0)/1000;
%         end
        fixObj = [fixImg.*d,d];
...
set(gcf,'color','w');
subplot(1,2,1)
plot(imageNoise,memm(:,4),'.','MarkerSize',20,'LineWidth',10)
xlabel("noise on image (pixel)",'FontSize',labelSize,'FontWeight','bold')
...
p1=fit(imageNoise,memm(:,4),'poly1')
hold on, pt2 = plot(imageNoise,p1(imageNoise),'r','LineWidth',5)

subplot(1,2,2)
plot(imageNoise,stdmm(:,4),'.','MarkerSize',20,'LineWidth',10)
xlabel("noise on image (pixel)",'FontSize',labelSize,'FontWeight','bold')
...
p2=fit(imageNoise,stdmm(:,4),'poly1')
hold on, pt2 = plot(imageNoise,p2(imageNoise),'r','LineWidth',5)
