...
stdmm = zeros(40,8);
memm = zeros(40,8);
depthNoisemm = zeros(40,1);
% A_depth
for j = 1:40
    j
    A_depth = 3*j;
    depthNoisemm(j) = A_depth;
...
set(gcf,'color','w');
subplot(1,2,1)
plot(depthNoisemm,memm(:,4),'.','MarkerSize',20,'LineWidth',10)
xlabel("noise on depth (mm)",'FontSize',labelSize,'FontWeight','bold')
...
% p1=fit(depthNoisemm,memm(:,4),'poly1')
% hold on, pt2 = plot(depthNoisemm,p1(depthNoisemm),'r','LineWidth',5)

subplot(1,2,2)
plot(depthNoisemm,stdmm(:,4),'.','MarkerSize',20,'LineWidth',10)
xlabel("noise on depth (mm)",'FontSize',labelSize,'FontWeight','bold')
...
% p2=fit(depthNoisemm,stdmm(:,4),'poly1')
% hold on, pt2 = plot(depthNoisemm,p2(depthNoisemm),'r','LineWidth',5)
